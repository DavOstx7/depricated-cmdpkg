...
def test_process_runner_settings():
...
def test_run_cmd(popen_class_mock: Mock, cmd_factory):
    cmd = cmd_factory()
    settings = ProcessRunnerSettings()
...
def test_run_command_with_stdin(stream_mock: Mock, popen_class_mock: Mock, cmd_factory, timeout_factory,
                                binary_io_factory):
...
@patch('subprocess.Popen')
@patch.object(DataStreamer, 'stream')
def test_run_command_without_stdin(stream_mock: Mock, popen_class_mock: Mock, cmd_factory, timeout_factory,
                                   binary_io_factory):
